...
float run_kernel(const char* kernel_name, KernelFunc invoke_kernel, float *d_A, float *d_B, float *d_C, int m, int k, int n, float *h_C, float *h_C_ref, std::mt19937 gen, int warmup_runs, int measurement_runs){
    cudaEvent_t beg, end;
    float elapsed_time;
    cudaEventCreate(&beg);
    cudaEventCreate(&end);
